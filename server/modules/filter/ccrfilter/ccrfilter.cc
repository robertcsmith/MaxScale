...
private:
    ...
    pcre2_match_data* m_md = NULL;                  /* PCRE2 match data */
...
private:
    ...
    LagStats    m_stats;
    pcre2_code* re = NULL;          /* Compiled regex text of match */
    pcre2_code* nore = NULL;        /* Compiled regex text of ignore */
...
CCRSession* CCRSession::create(MXS_SESSION* session, CCRFilter& instance)
{
    ...
    if (new_session)
    {
        if (instance.ovector_size)
        {
            new_session->m_md = pcre2_match_data_create(instance.ovector_size, NULL);
            if (!new_session->m_md)
            {
                delete new_session;
                new_session = NULL;
